...
class AE_with_imputation_ML_1m():

    def __init__(self, num_user, num_item, train, test, output, n_hidden, **params):
        super(AE_with_imputation_ML_1m, self).__init__()
    ...
    def get_cost_ranking_updates(self):
    ...
    def preTrain(self, iterations=200, batch_size=6040):
        ...
        for i in range(self._num_user) :
            ...
            constrain = sorted_missing[int(round(self._num_item *0.0035 ))]
    ...
    def estimate(self, iterations=300, batch_size=6040):
        ...
        train_da = theano.function(
            ...
            updates=updates,
            givens={
                self.x: self.output[index * batch_size: (index + 1) * batch_size],
                self.o: self.output[index * batch_size: (index + 1) * batch_size]
            }
        )
        ...
        train_preds = self.predict(self.output)
...
def example():
    ...
    mf_model = AE_with_imputation_ML_1m(
    ...
    mf_model = AE_with_imputation_ML_1m(
        output_matrix.shape[0], num_item, train_matrix, test_matrix, output_matrix, num_hidden)
